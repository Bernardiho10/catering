...
export const Particles = ({
    ...
    const drawParticles = () => {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            drawCircle(circle, particleColor);
            circles.current.push(circle);
        }
    };

    const drawCircle = (circle: any, rgb: number[], update = false) => {
        if (context.current) {
    ...
    const animate = () => {
        ...
        circles.current.forEach((circle: any, i: number) => {
            ...
            if (circle.x < -circle.size || circle.x > canvasSize.current.w + circle.size || circle.y < -circle.size || circle.y > canvasSize.current.h + circle.size) {
                ...
                drawCircle(newCircle, hexToRgb(color));
                // circles.current.push(newCircle);
            } else {
                drawCircle(circle, hexToRgb(color), true);
            }
        });
